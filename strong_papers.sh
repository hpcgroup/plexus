...
if ! [[ "$NNODES" =~ ^[0-9]+$ ]] || (( NNODES < 1 )); then
    echo "Invalid NNODES: '$NNODES' (SLURM_NNODES='${SLURM_NNODES}', SLURM_JOB_NUM_NODES='${SLURM_JOB_NUM_NODES}')"
    exit 1
fi
...
# PARTITIONED_DATA_DIR=/pscratch/sd/c/cunyang/gnn/plexus/dataset/papers/papers_part${PARTITIONS_PER_DIM}
...
echo $run_cmd
eval $run_cmd > strong/papers-100m/papers_G_INTRA_R${G_INTRA_R}_G_INTRA_C${G_INTRA_C}_G_INTRA_D${G_INTRA_D}_G_DATA${G_DATA}_ratio${ratio}.log 2>&1
