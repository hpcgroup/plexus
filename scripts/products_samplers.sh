...
export MASTER_PORT=29525 WORLD_SIZE=4 CUDA_VISIBLE_DEVICES=3,2,1,0 MPICH_GPU_SUPPORT_ENABLED=0
...
run_one () {
  ...
  timeout 900 srun -N 1 -n 4 -c 32 --cpu-bind=cores --gpus-per-node=4 ./get_rank.sh \
    ...
    --data_dir /pscratch/sd/c/cunyang/gnn/plexus/dataset/products/products_part8 \
    "$@" > result/small_acc/products_${tag}.log 2>&1
  echo "products_${tag}: exit=$? best=$(grep 'TEST: acc' result/small_acc/products_${tag}.log | awk '{print $3}' | tr -d ',' | sort -rn | head -1)"
}
echo "== products: coverage 5 / 2.5 / 0.5 =="
run_one r0.1_uni  0.1  uniform
run_one r0.1_hub  0.1  hub --minibatch_hub_frac 0.3
run_one r0.05_uni 0.05 uniform
run_one r0.05_hub 0.05 hub --minibatch_hub_frac 0.3
